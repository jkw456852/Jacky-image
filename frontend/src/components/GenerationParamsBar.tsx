...
export function GenerationParamsBar({ value, onChange, size = 'xs', className }: GenerationParamsBarProps) {
  const [modelOptions, setModelOptions] = useState<ModelOption[]>(() => getModelOptions());
  ...
  useEffect(() => {
    const refreshModels = () => setModelOptions(getModelOptions());
  ...
  const handleModelChange = (newModel: ModelId) => {
    ...
    onChange({
      ...
      webSearchEnabled: supportsWebSearchGrounding(newModel) ? value.webSearchEnabled : false,
      imageSearchEnabled: supportsImageSearchGrounding(newModel) ? value.imageSearchEnabled : false,
    });
  };
  ...
  return (
    <div className={cn('flex flex-wrap items-center gap-1.5', className)}>
      ...
      <GeminiSearchGroundingControl
        ...
        imageSearchEnabled={value.imageSearchEnabled}
        onChange={onChange}
        size={size}
      />
