...
const MOBILE_HINT_STORAGE_KEY = 'jacky-gif-tuner-mobile-hint-hidden';
...
export function GifFrameTuner({ cells, cellWidth, cellHeight, onGenerate, onClose }: GifFrameTunerProps) {
  ...
  useEffect(() => {
    ...
    try {
      if (getDesktopPreference(MOBILE_HINT_STORAGE_KEY) === '1') return;
    } catch {
  ...
  const dismissMobileHint = useCallback(() => {
    if (hideMobileHintNextTime) {
      try {
        setDesktopPreference(MOBILE_HINT_STORAGE_KEY, '1');
      } catch {
        // Ignore unavailable storage.
      }
    }
