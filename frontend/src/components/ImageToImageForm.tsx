...
export function ImageToImageForm({
  ...
  const disabledMessage = '请先在设置中配置 Jacky API 密钥，配置完成后即可开始转换图片。';
  ...
  const maskSupported = maxImages > 0;
  ...
  const handleOptimize = useCallback(() => {
    ...
    const handle = streamPromptOptimize(
      { modelConfigId: textModel.id, model: textModel.id, mode: 'image-to-image', prompt: prompt.trim(), images },
      {
  ...
  const handleModelChange = (newModel: ModelId) => {
    setModel(newModel);
    setWebSearchEnabled(current => supportsWebSearchGrounding(newModel) && current);
    setImageSearchEnabled(current => supportsImageSearchGrounding(newModel) && current);
  ...
  const handleSubmit = () => {
    if (prompt.trim() && pendingFiles.length > 0 && (!maskDraft || processedMask)) {
      onSubmit({
        prompt: prompt.trim(),
        files: pendingFiles,
        outputSize,
  ...
  const handleMaskFile = useCallback(async (file: File) => {
    ...
    setMaskError(null);
    try {
      const draft = await createMaskDraft(file);
      setMaskDraft(draft);
    } catch (error) {
  ...
  useEffect(() => {
    const targetDataUrl = pendingFiles[0]?.dataUrl;
  ...
  useEffect(() => {
    ...
    setPendingFiles(prev => {
      ...
      const remainingSlots = Math.max(0, effectiveMaxImages - prev.length);
      if (remainingSlots <= 0) {
  ...
  }, [effectiveMaxImages, maxImages, model, onDraftConsumed, referenceDraft?.id]);
  ...
  const processFiles = useCallback(async (fileList: FileList | File[]) => {
    ...
    // 检查图片数量限制
    if (pendingFiles.length + filesToProcess.length > effectiveMaxImages) {
      const modelLimit = MODEL_IMAGE_LIMITS[model];
      const maskNote = maskDraft && maskStrategy.consumesImageSlot ? '（蒙版占用 1 个图片输入位）' : '';
      setUploadError(`${MODEL_OPTIONS.find(o => o.value === model)?.label} ${modelLimit.description}${maskNote}`);
      return;
    }
  ...
  }, [pendingFiles.length, effectiveMaxImages, autoLayoutLocked, model, detectImageAspectRatio, maskDraft, maskStrategy.consumesImageSlot]);

  const handleImportAssets = useCallback(async (selectedAssets: ImageAsset[]) => {
    if (selectedAssets.length === 0) return;

    const remainingSlots = Math.max(0, effectiveMaxImages - pendingFiles.length);
    if (remainingSlots <= 0) {
      setUploadError(`${MODEL_OPTIONS.find(o => o.value === model)?.label} 最多支持 ${maxImages} 张参考图`);
      return;
    }
  ...
  }, [autoLayoutLocked, detectImageAspectRatio, effectiveMaxImages, maxImages, model, pendingFiles.length]);
  ...
  const canSubmit = prompt.trim().length > 0
    ...
    && maxImages > 0
    && pendingFiles.length <= effectiveMaxImages
    && (!maskDraft || Boolean(processedMask))
    && !maskConsumesOverflow
    && !disabled
    && !loading
    && !maskLoading;
  ...
  return (
    <div ref={formRef} className="space-y-4">
        ...
        {disabled ? (
          ...
          <>
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {pendingFiles.length} / {effectiveMaxImages} 张
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setAssetPickerOpen(true)}
                  disabled={loading || pendingFiles.length >= effectiveMaxImages}
                  title="从素材库导入参考图"
            ...
            <MaskUploadControl
              ...
              disabled={!maskSupported || loading}
          ...
          <div className="ml-auto flex w-full justify-end gap-2 px-4 pb-2 sm:w-auto">
            ...
            <Button
              ...
              disabled={disabled || (!prompt.trim() && pendingFiles.length === 0 && !maskDraft)}
              title="清空提示词和图片"
            >
              <X className="w-5 h-5" />
            </Button>
      ...
      <AgentAssetPickerDialog
        ...
        maxSelected={Math.min(MAX_ASSET_IMPORTS, Math.max(1, effectiveMaxImages - pendingFiles.length))}
