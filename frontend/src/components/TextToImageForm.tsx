...
interface TextToImageFormProps {
  wideMode?: boolean;
  onSubmit: (data: { prompts: string[]; outputSize: OutputSize; customSize?: string; aspectRatio: AspectRatio; temperature: number; webSearchEnabled: boolean; imageSearchEnabled: boolean; model: string; gptImageQuality: GptImageQuality; gptImageStyle: GptImageStyle; gptImageBackground: GptImageBackground; parallelCount: ParallelCount }) => void;
  disabled?: boolean;
...
export function TextToImageForm({ onSubmit, disabled = false, onDraftConsumed, onConfigureApiKey, initialData }: TextToImageFormProps) {
  ...
  const handleOptimize = useCallback(() => {
    ...
    const handle = streamPromptOptimize(
      { modelConfigId: textModel.id, model: textModel.id, mode: 'text-to-image', prompt: prompt.trim() },
      {
        onDelta(token) {
