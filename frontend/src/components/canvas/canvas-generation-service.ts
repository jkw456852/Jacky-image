 ...
 * 仅使用 host 的 jacky-task-client（createJackyTask / getJackyTask / ackJackyTask）。
 * 视频/音频不在范围内；图生图无 mask（队列不支持）。
 */
import { ackJackyTask, createJackyTask, getJackyTask, resolveImageTaskProvider, type JackyTaskResponse, type JackyTaskStatus, type ImageReference } from "@/lib/ccode-task-client";
...
export async function submitNodeGeneration(args: {
  ...
  const taskId = await createJackyTask({
    ...
    webSearchEnabled: Boolean(args.config.webSearchEnabled),
    imageSearchEnabled: Boolean(args.config.imageSearchEnabled),
...
export async function pollNodeTask(
  taskId: string,
  onStatus: (status: JackyTaskStatus) => void,
  signal?: AbortSignal,
...
export async function checkExistingTask(taskId: string): Promise<{ status: JackyTaskResponse["status"]; images?: CanvasGeneratedImage[]; error?: string }> {
  const task = await getJackyTask(taskId);
...
/** 结果可能是 data URL 或 `URL:/api/jacky/images/...`；统一下载为 blob 存入本地 IndexedDB。 */
