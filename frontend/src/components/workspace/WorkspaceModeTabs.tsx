...
import { Armchair, Bot, Film, Frame, Images, LibraryBig, ScanSearch, Sparkles, WandSparkles } from 'lucide-react';
...
const tabs = [
  ...
  { value: 'advanced-repaint', icon: WandSparkles, label: '高级重绘' },
  { value: 'seat-cover-generation', icon: Armchair, label: '座套生成' },
...
export function WorkspaceModeTabs({ wideMode = false, showPromptGallery = false }: WorkspaceModeTabsProps) {
  ...
  return (
    <TabsList
      className="scrollbar-hide flex h-16 w-full max-w-full touch-pan-x select-none justify-start gap-1 overflow-x-auto overflow-y-hidden overscroll-x-contain rounded-2xl bg-muted p-1 sm:grid sm:overflow-visible sm:border sm:border-border sm:select-auto"
