...
import { SeatCoverGenerationWorkspace } from '@/components/seat-cover-generation/SeatCoverGenerationWorkspace';
...
import { getJackyTask } from '@/lib/ccode-task-client';
...
import { clearDesktopTaskSource, publishDesktopTaskSource } from '@/lib/desktop-task-status';

const WORKSPACE_TASK_SOURCE = 'workspace-jobs';

function getWorkspaceTaskTitle(job: StoredJob): string {
  if (job.mode === 'image-to-image') return '图生图';
  if (job.mode === 'prompt-gallery') return '提示词画廊生图';
  return '文生图';
}
...
export function WorkspaceShell() {
  ...
  const [activeTab, setActiveTab] = useState<'image-generation' | 'advanced-repaint' | 'seat-cover-generation' | 'agent' | 'canvas' | 'assets' | 'reverse-prompt' | 'gif' | 'prompt-gallery'>('agent');
  ...
  const desktopWorkspaceTasks = useMemo(() => workspace.jobs.slice(0, 200).map(job => ({
    id: `workspace:${job.id}`,
    status: job.status === 'queued' || job.status === '排队中'
      ? 'queued' as const
      : job.status as 'processing' | 'completed' | 'failed',
    title: getWorkspaceTaskTitle(job),
    detail: job.prompt,
    count: job.parallelCount,
    updatedAt: job.created_at,
  })), [workspace.jobs]);

  useEffect(() => {
    publishDesktopTaskSource(WORKSPACE_TASK_SOURCE, desktopWorkspaceTasks);
  }, [desktopWorkspaceTasks]);

  useEffect(() => () => clearDesktopTaskSource(WORKSPACE_TASK_SOURCE), []);
  ...
  const handleCheckStatus = useCallback(async (job: StoredJob) => {
    ...
    try {
      const task = await getJackyTask(job.serverTaskId);
  ...
  const generationInitialData = useMemo(() => (
    workspace.retryData
      ? {
        ...
        webSearchEnabled: workspace.retryData.webSearchEnabled,
        imageSearchEnabled: workspace.retryData.imageSearchEnabled,
  ...
  return (
    <div
      className={cn(
        'jacky-workspace-shell mx-auto flex min-h-screen w-full flex-col gap-4 overflow-x-hidden px-3 py-3 transition-[max-width] duration-200 sm:gap-5 sm:px-6 sm:py-5 lg:px-8',
    ...
    >
      <div className={cn(
        'jacky-workspace-surface flex-1 bg-transparent shadow-none sm:rounded-3xl sm:bg-card/95 sm:shadow-sm sm:border sm:border-border/70',
        wideMode && 'flex min-h-0 flex-col',
        !wideMode && activeTab === 'agent' && 'flex min-h-0 flex-col'
      )}>
        <div className={cn(
          'jacky-workspace-content p-0 sm:p-5',
          wideMode
          ...
          >
              ...
              {wideMode && (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <h2 className="truncate text-base font-semibold tracking-tight leading-tight">Jacky Image</h2>
            ...
            <div className={cn(
              ...
              wideMode && (activeTab === 'image-generation' || activeTab === 'advanced-repaint' || activeTab === 'seat-cover-generation' || activeTab === 'agent'
                ? 'xl:overflow-hidden'
              ...
              <TabsContent value="advanced-repaint" keepMounted className={cn('min-h-0', wideMode ? 'xl:flex xl:min-h-0 xl:flex-1 xl:flex-col' : 'space-y-4')}>
              ...
              <TabsContent value="seat-cover-generation" keepMounted className={cn('min-h-0', wideMode ? 'xl:min-h-0 xl:flex-1 xl:overflow-y-auto xl:pr-1' : 'space-y-4')}>
                <SeatCoverGenerationWorkspace
                  hasApiKey={workspace.hasApiKey}
                  onConfigureApiKey={() => setSettingsOpen(true)}
                  showToast={showToast}
                />
