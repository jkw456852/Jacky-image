...
import { cancelJackyTask, createJackyTask, getJackyTask, resolveImageTaskProvider, type ImageReference } from '@/lib/ccode-task-client';
...
import { getCompleteImageModels, loadRegistry } from '@/lib/jacky-models';
import {
  streamAgentChat,
  describeImage,
...
import { supportsAgentNativeWebSearch } from '@/lib/jacky-text-protocol';
import { getDesktopPreference, setDesktopPreference } from '@/lib/desktop-preferences';
...
export function useAgentChat() {
  ...
  const [webSearchEnabled, setWebSearchEnabled] = useState(() =>
    getDesktopPreference('jacky-agent-web-search') === 'true'
  );
  const [intentRecognition, setIntentRecognition] = useState(() =>
    getDesktopPreference('jacky-agent-intent-recognition') !== 'false'
  );
  ...
  const getAgentTextModelConfig = useCallback(() => {
    ...
    if (!configured?.apiKeyConfigured || !configured.baseUrl || !configured.modelId) {
  ...
  const ingestImage = useCallback(async (
    ...
    // 上传图片（有 contentHash）已在 prepareUploadImage 时存于 jacky-upload-cache，
    // 不再重复存到 jacky-image-db，节省空间；生成图片无 contentHash 则照常存储。
  ...
  const stopStreaming = useCallback(() => {
    const taskIdToCancel = generatingTaskId;
    ...
    void clearPendingProposal();
    void clearPendingGeneration();
    if (taskIdToCancel) void cancelJackyTask(taskIdToCancel).catch(() => undefined);
  }, [flushAndCancelRaf, generatingTaskId]);
  ...
  const toggleWebSearch = useCallback(() => {
    ...
    setWebSearchEnabled(prev => {
      const next = !prev;
      try { setDesktopPreference('jacky-agent-web-search', String(next)); } catch { /* ignore */ }
      return next;
    });
  ...
  const toggleIntentRecognition = useCallback(() => {
    setIntentRecognition(prev => {
      const next = !prev;
      try { setDesktopPreference('jacky-agent-intent-recognition', String(next)); } catch { /* ignore */ }
      return next;
    });
  }, []);
