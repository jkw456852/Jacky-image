...
import { createJackyTask, getJackyTask, ackJackyTask, resolveImageTaskProvider, type ImageReference } from '@/lib/ccode-task-client';
...
export function useGifWorkflow(): UseGifWorkflowResult {
  ...
  const loadGridImageUrl = useCallback(async (target: ActiveGifJob): Promise<string | null> => {
    ...
    if (ref.startsWith('IDB:') || ref.startsWith('FILE:') || ref.startsWith('blob:')) {
  ...
  const finalizeGrid = useCallback(async (
    ...
    try { await ackJackyTask(serverTaskId); } catch { /* ignore */ }
  ...
  const subscribeServerTask = useCallback((taskId: string) => {
    clearSubscription();
    const unsubscribe = jackyTaskSocket.subscribeTask(taskId, task => {
      const current = jobRef.current;
  ...
  useEffect(() => {
    ...
    if (initial.status === 'generating_grid' && initial.serverTaskId) {
      ...
      getJackyTask(initial.serverTaskId)
        .then(task => {
          const current = jobRef.current;
  ...
  const encodeGif = useCallback(async (params: GifEncodeParams) => {
    ...
    try {
      ...
      setGifBlob(blob);
      await triggerGifDownload(blob, `gif-${current.id}.gif`);
  ...
  const encodeTunedGif = useCallback((frames: ImageData[], params: GifEncodeParams) => {
    ...
    try {
      ...
      setGifBlob(blob);
      void triggerGifDownload(blob, `gif-${current.id}.gif`);
      updateJob(prev => ({ ...prev, status: 'done', updatedAt: nowIso() }));
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      updateJob(prev => ({ ...prev, status: 'failed', error: message, updatedAt: nowIso() }));
  ...
  const downloadGif = useCallback(() => {
    ...
    void triggerGifDownload(gifBlob, `gif-${current.id}.gif`);
  }, [gifBlob]);
  ...
  const refreshFromServer = useCallback(async (onStatus?: (message: string) => void) => {
    ...
    onStatus?.('正在查询任务状态…');
    try {
      const task = await getJackyTask(current.serverTaskId);
      if (task.status === 'completed') {
