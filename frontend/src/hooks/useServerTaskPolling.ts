...
import type { JackyTaskResponse } from '@/lib/ccode-task-client';
 ...
 * - 每个等待中的 serverTaskId 走 jackyTaskSocket.subscribeTask
 ...
 * socket 自身负责重连、心跳、HTTP 兜底，详见 jacky-task-socket.ts。
 */
export function useServerTaskPolling(
  ...
  useEffect(() => {
    ...
    for (const job of jobs) {
      ...
      const handler = (task: JackyTaskResponse) => {
      ...
      const unsubscribe = jackyTaskSocket.subscribeTask(taskId, handler);
