...
export const WIDE_MODE_STORAGE_KEY = 'jacky-wide-mode';
...
function readStoredWideMode(): boolean {
  ...
  try {
    return getDesktopPreference(WIDE_MODE_STORAGE_KEY) === 'enabled';
  } catch {
    return false;
  }
}
...
function writeStoredWideMode(enabled: boolean): void {
  ...
  try {
    ...
    setDesktopPreference(WIDE_MODE_STORAGE_KEY, value);
  } catch {
