...
export function useWorkspaceJobs() {
  ...
  const [loadedImages, setLoadedImages] = useState<Set<string>>(
    () => new Set(jobs.filter(job => job.images?.length || job.imageData).map(job => job.id)),
  );
  ...
  useEffect(() => {
    ...
    if (stored.length > 0) {
      ...
      const needsLegacyImageMigration = stored.some(job => job.status === 'completed' && !job.images?.length && !job.imageData);
      if (desktopRecords && !needsLegacyImageMigration) {
        return;
      }
      openDB()
        .then(db => {
          ...
          request.onsuccess = () => {
            ...
            setJobs(prev => {
              ...
              if (desktopRecords) saveJobs(next);
              return next;
            });
