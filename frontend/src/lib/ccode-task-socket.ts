import { getJackyTask, getJackyQueueStatus, type JackyTaskResponse, type JackyQueueStatus } from '@/lib/ccode-task-client';

type TaskUpdateHandler = (task: JackyTaskResponse) => void;
type QueueUpdateHandler = (stats: JackyQueueStatus) => void;
...
function buildSocketUrl(): string | null {
  ...
  try {
    const url = new URL('/api/jacky/ws', window.location.href);
...
class JackyTaskSocket {
  ...
  private async fetchQueueOnce(): Promise<void> {
    try {
      const stats = await getJackyQueueStatus();
      for (const handler of this.queueHandlers) {
        try { handler(stats); } catch { /* ignore handler error */ }
      }
    } catch {
...
export const jackyTaskSocket = new JackyTaskSocket();
/** @deprecated Use jackyTaskSocket */
export const ccodeTaskSocket = jackyTaskSocket;
