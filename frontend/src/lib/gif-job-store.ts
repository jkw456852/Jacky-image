...
import { getDefaultImageModel, getCompleteImageModels, loadRegistry } from '@/lib/jacky-models';
import {
...
const STORAGE_KEY = 'jacky-gif-active-job';
...
export function loadActiveGifJob(): ActiveGifJob | null {
  ...
  try {
    const raw = getDesktopPreference(STORAGE_KEY);
    if (!raw) return null;
...
export function saveActiveGifJob(job: ActiveGifJob | null): void {
  ...
  try {
    if (!job) {
      removeDesktopPreference(STORAGE_KEY);
      return;
    }
    setDesktopPreference(STORAGE_KEY, JSON.stringify(job));
  } catch {
