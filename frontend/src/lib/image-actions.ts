...
const TOAST_EVENT = 'jacky-image-action-toast';
const USE_AS_REFERENCE_EVENT = 'jacky-use-as-i2i-reference';
...
function stripImageRef(ref: string): string {
  if (ref.startsWith('URL:') || ref.startsWith('MULTI_URL:') || ref.startsWith('IDB:') || ref.startsWith('FILE:') || ref.startsWith('blob:')) {
...
async function resolveStoredRefToBlob(payload: ImageActionPayload): Promise<Blob | null> {
  ...
  if (!ref) return null;
  if (ref.imageRef.startsWith('IDB:') || ref.imageRef.startsWith('FILE:')) {
    const key = ref.imageRef.substring(ref.imageRef.indexOf(':') + 1);
...
function getDownloadName(payload: ImageActionPayload, blob: Blob): string {
  const rawName = payload.name?.trim() || payload.id || payload.assetId || payload.agentImageId || `jacky-image-${Date.now()}`;
  const baseName = rawName.replace(/[\\/:*?"<>|]+/g, '-').replace(/\s+/g, '-').slice(0, 80) || 'jacky-image';
...
export async function downloadImagePayload(payload: ImageActionPayload): Promise<void> {
  ...
  await saveBlobToDownloads(blob, getDownloadName(payload, blob));
...
export async function runImageAction(
  ...
  try {
    if (action === 'download') {
      ...
      dispatchImageActionToast(window.jackyDesktop?.downloads ? '图片已保存到本地下载目录' : '图片已开始下载', 'success');
      return;
    }
    if (action === 'copy') {
