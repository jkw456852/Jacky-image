...
export interface JackyModelRegistry {
...
function normalizeImageModelConfig(raw: Partial<ImageModelConfig> & { apiKey?: unknown }): ImageModelConfig | null {
  ...
  return {
    ...
    apiKeyConfigured: Boolean(raw.apiKeyConfigured || String(raw.apiKey || '').trim()),
...
function normalizeTextModelConfig(raw: Partial<TextModelConfig> & { apiKey?: unknown }): TextModelConfig | null {
  const id = String(raw.id || '').trim();
  if (!id) return null;
...
function getInitialRegistry(): JackyModelRegistry {
  return {
    imageModels: [],
    textModels: [],
...
function normalizeRegistry(parsed: Partial<JackyModelRegistry>): JackyModelRegistry {
...
export function loadRegistry(): JackyModelRegistry {
  if (typeof window === 'undefined') {
    return getInitialRegistry();
  }

  const desktopBridge = window.jackyDesktop?.modelRegistry;
  if (!desktopBridge) return getInitialRegistry();

  try {
    const registry = desktopBridge.load();
    if (registry && typeof registry === 'object') {
      return normalizeRegistry(registry as Partial<JackyModelRegistry>);
    }
  } catch {
    return getInitialRegistry();
  }
...
export function saveRegistry(registry: JackyModelRegistry): void {
  ...
  const desktopBridge = window.jackyDesktop?.modelRegistry;
  if (!desktopBridge) {
    throw new Error('Jacky Image 仅支持桌面版配置存储');
  }
  const result = desktopBridge.save(normalized);
  if (!result?.ok) {
    throw new Error(result?.error || '无法写入桌面安全配置文件');
  }
}

export function getImageModelById(registry: JackyModelRegistry, id: string): ImageModelConfig | undefined {
  return registry.imageModels.find((model) => model.id === id);
}

export function getTextModelById(registry: JackyModelRegistry, id: string): TextModelConfig | undefined {
  return registry.textModels.find((model) => model.id === id);
}

export function getDefaultImageModel(
  registry: JackyModelRegistry,
...
export function getCompleteImageModels(registry: JackyModelRegistry): ImageModelConfig[] {
  return registry.imageModels.filter(isCompleteImageModel);
}

export function getCompleteTextModels(registry: JackyModelRegistry): TextModelConfig[] {
