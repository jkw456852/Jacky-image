...
import { getImageModelById, loadRegistry } from '@/lib/jacky-models';
...
function fitGptImageResolutionToLimits(
  width: number,
  height: number,
  ratioWidth: number,
  ratioHeight: number,
): string | undefined {
  const maxSide = getCustomSizeMaxSide('gpt-image-2') || 3840;
...
export function findReferenceCapableModel(preferredId?: string): ModelId | null {
  ...
  const complete = registry.imageModels.filter((model) =>
    Boolean(model.name.trim() && model.modelId.trim() && model.apiKeyConfigured && model.baseUrl.trim()),
  );
...
export function getCompatibleRetryData(job: StoredJob): RetryData {
  ...
  return {
    ...
    webSearchEnabled: supportsWebSearchGrounding(model) && Boolean(job.webSearchEnabled),
    imageSearchEnabled: supportsImageSearchGrounding(model) && Boolean(job.imageSearchEnabled),
...
function getGoogleImageModelIdentifiers(model: string): string[] {
  const configured = getModelConfig(model);
  return [model, configured?.builtinPreset, configured?.modelId]
    .filter((value): value is string => Boolean(value));
}

export function supportsWebSearchGrounding(model: string): boolean {
  return getGoogleImageModelIdentifiers(model).some(identifier => (
    identifier === 'gemini-3-pro-image-preview'
    || identifier === 'gemini-3-pro-image'
    || identifier === 'gemini-3.1-flash-image-preview'
    || identifier === 'gemini-3.1-flash-image'
  ));
}

export function supportsImageSearchGrounding(model: string): boolean {
  return getGoogleImageModelIdentifiers(model).some(identifier => (
    identifier === 'gemini-3.1-flash-image-preview'
    || identifier === 'gemini-3.1-flash-image'
  ));
