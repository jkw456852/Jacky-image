// 反推提示词流式客户端
// 统一通过 /api/jacky/proxy/text 按文本协议转发。
...
export function streamReversePrompt(
  ...
  const promise = (async () => {
    try {
      ...
      await streamTextProtocol(protocol, resolvedBaseUrl, { ...input, model: resolvedModelId, modelConfigId: configuredModel?.id || input.model }, callbacks, controller.signal);
    } catch (err) {
...
async function streamTextProtocol(
  ...
  const response = await fetch('/api/jacky/proxy/text', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      modelConfigId: input.modelConfigId || input.model,
      stream: true,
      requestBody: body,
    }),
    signal,
