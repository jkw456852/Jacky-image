// 反推提示词功能的模型与模式配置
// 模型列表从 jacky-models 注册表动态读取，不再硬编码
// system prompt 模板保留在此文件

import type { TextModelConfig } from '@/lib/jacky-models';
import { getDefaultConfiguredTextModel, getConfiguredTextModel } from '@/lib/model-endpoints';
import { getTextProviderDescription } from '@/lib/jacky-text-protocol';
...
export function getReversePromptModelOptionsList(): ReversePromptModelOption[] {
  ...
  const { loadRegistry, getCompleteTextModels } = require('@/lib/jacky-models');
