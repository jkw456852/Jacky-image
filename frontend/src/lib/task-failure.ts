import type { JackyTaskResponse, JackyTaskStatus } from '@/lib/ccode-task-client';
 ...
 */
export function classifyTaskFailure(task: Pick<JackyTaskResponse, 'status' | 'error'>): FailureClassification {
  const status = task.status as JackyTaskStatus;
